...
好友码、申请状态机、删除/屏蔽、文字私聊、历史补查、失败重试、未读及白板消息弹层已实现。本轮补齐设备密钥/安全卡信任、HPKE Auth 邀请、接受/拒绝/撤销/补发、白板加入和 joined 回报。两个社交开关在代码中仍默认关闭；2026-09-23 按用户明确部署要求在生产开启，Web/API 已发布，见 [上线记录](2026-09-23-social-deployment.md)。鸿蒙实机验收按用户要求由队员执行。
...
项目已有 `cryptography 2.9.0`（Apache-2.0）与 `crypto`。新增 `HpkeAuth` 严格实现 RFC 9180 的单次 Auth/X25519/HKDF-SHA256/AES128GCM 封装，每次使用新临时密钥且仅 seq=0；没有上下文重用、多消息序号、exporter 或算法协商。未新增运行时密码依赖；此实现尚未经历独立外部安全审计，部署不等同于审计完成。
...
Web/API 发布及线上冒烟已执行，详见上线记录；生产回退未实际演练，鸿蒙与实体双端验收仍待完成。
