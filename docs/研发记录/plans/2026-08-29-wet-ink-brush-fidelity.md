...
**迭代（同日用户复验）**：首版窗口 250ms 产生新缺陷——补偿在实测压力到位前撤退，每笔出现"粗起笔收窄成尖"的反向凹谷。教训：**补偿窗口必须长于起笔压力爬升期**（实测 0.5-1.2s）→ 调至 1500ms，实测压力在包络仍处高位时无缝接管，max(实测, 包络) 全程近平坦无跳变。权衡：刻意轻笔画的起笔 1.5s 内偏宽（渐变收窄，几何 startTaper 仍提供笔锋造型）。攻击测试用例同步重写为覆盖爬升期断言。

**迭代二（同日用户复验）**：1500ms 全局包络引入新回归——**钢笔没粗细变化**。根因：包络是 `max(实测, 包络)`，轻力度书写者实测映射压力多在 0.3-0.45，而包络从 0.50 起衰减，写字这类 0.4-0.6s 短笔画**全程被水位抬压**，粗细动态（钢笔的表现力所在）被压平；毛笔/铅笔未报异常系用户写毛笔时力度大、实测很快越过包络。教训：**起笔闪变是笔形渲染特性决定的**——低压起笔渲染为细线的笔形（毛笔/铅笔 thinning 强）才会闪变；钢笔低压力渲染为宽线、收窄自然，无需补偿。修复：攻击补偿参数从 `InputPolicy`（设备级）移至 `StrokeInputModeler` 构造参数（逐笔级），控制器落笔建模器创建时按笔形白名单传入——仅毛笔/铅笔启用 1500ms/0.50，钢笔/圆珠笔/荧光笔 0=关闭，实测压力原样编码。新增门禁：模型器构造开关分野 1 例 + 控制器级笔形回归 2 例（钢笔全程不被抬到水位 `lessThan(0.5)`、毛笔/铅笔起笔编码域恰达 0.5，依据 encodePressure 单调增仿射保序），全量 **847 全绿**（e7d8922）。
