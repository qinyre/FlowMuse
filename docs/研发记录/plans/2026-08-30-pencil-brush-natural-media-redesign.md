...
| 进度 | T0～T3：732fa68/4719264/c2aca40/9902924/69320bf；T4：5fbc93a（+26505be 补 bounds/opacity/dim 验收、4dd3d90 lint）；T5：625d142；T6：d2a7f46。T4 为满足 §3.2 B1/B2 同入参预览=提交门禁，提前落地 T6 工作项 6 的通路与断言：`ToolOverlay.creationStrokeId` 携带 live element id → `buildPreviewElement` freedraw 预览元素用该 id（与 `_buildElement` 提交元素同 id 同种子），fidelity A5 断言"live strokeId 与最终 ElementId 必须相同"；另修 `customDataWithFreedrawRender` classicV1 显式移除 v2 标记（此前 spread 保残留导致显式降级无效）。T5 期间发现并修复：①分块交界 join(k−1→k) 两块都不发（并集缺 key）——按 §3.4"较后 edge 拥有"补块首入口 join 并以测试锁定；②轻压可见下限 0.7px（视觉审查抓到 S 曲线负压段 0.96px 全宽在斜向 AA 下断线成 6 列白点，违反 §3.5"最低有效宽度仍可见"；冻结曲线测试同步更新，只影响 p≲0.012）；③brushSCurve fixture 压力 0.30+0.40·sin 谷值 −0.10 越界，改 0.35+0.35·sin（峰值不变）。T5 有意偏差：圆帽用对称 4 点近似替代 spike 的单侧近似；转角尖刺验收以 plan 级 join 突出度断言（像素法向扫描在 90° 转角沿另一条腿读到假宽度 76px）。T6：ActiveFreedrawView 冻结 renderVersion、layered painter 同 dispatch、90% mask ≤1%、前缀 key 稳定、提交沿 live id（5 测试）；本地无预测点机制，工作项 4/5 以前缀 key 稳定性等价覆盖并记录。T11（验收矩阵）：N1（v1 摘要锁，各任务 v1-lock 测试）、N2-N5（T4 铅笔测试）、N6-N8（T5 毛笔测试）、N9/N10/N13/N19 补集入 natural_media_acceptance_matrix_test（N8 收严 ≤45%、N9 短划可见、N10 100 次摘要、N13 远端 35% 透明度 98% 弧长 mask ≤2% 无加重带、N19 线性——阈值按计划证据条款修订：实测 2k=1.7/4k=3.0/8k=9.0/16k=18.6ms，8× 点数 11× 耗时属轻度超线性分配而非二次，基线 2k、阈值 24=实测×2.2 裕量，二次回归仍会爆表）、N11/N12（T6/T7）、N14（T1/T2 契约 + 场景库以序列化文本原样存 SQLite content 列，语义由序列化层保证）、N15（LiveInkStyle 缺字段回退 + T7-d 版本翻转拒段）、N16（T9 SVG 测试 + Chromium 实测记录）、N17（T8 bounds 墨迹包络锁 + scene/export/culling 单一真源）、N18（T4/T5 结构 + 既有远端稳定帧 tail 有限测试）、N20（既有 16k/64-stroke 内存与 bounds 门禁 + 粒子封顶测试）、N21（既有 focus dim 测试：只包 alpha 不污染 Picture）、N22（T3 modeler v2 稳定器测试，轻压 1.5s 不抬 0.5）。685 测试全绿。T10（笔盒）：压力滑块标签按笔形映射（铅笔=浓淡响应、毛笔=提按响应、其余=压感既有语义）+ v2 笔形模拟压感可解释文案（映射抽为公共 pressureLabelFor/simulatedPressureCaptionFor 并测试；无未实现控件、无能力承诺文案）；各笔偏好持久化/切笔不串改为既有行为（回归覆盖）。T9（SVG/格式）：SVG v2 分支消费共享 plan（bodyPolygon/basePolygon 从渲染器抽出共用真源，不复制 sampler/seed/滤波）；铅笔=基底多边形+≤3 密度桶复合 path（颗粒为旋转四边形）、毛笔=包络+毫丝描边线；SVG 侧确定性抽稀（多边形≤8000 顶点、颗粒≤4000、毫丝≤2000，等步长、末点保留）满足 16k 点预算（产物落盘 build/natural_media_baseline/svg_v2/）；v1 导出结构不变（outline+pattern 2 path）。修正隐藏缺陷：毫丝在画布上 fill 语义零面积不可见→改描边 0.8px round cap（画布与 SVG 同口径）。Chromium 实测（Playwright+页面 canvas 栅格）：铅笔轻重宽度同 6px、暗度 1.72→3.04（与 Canvas N2 同向）；毛笔宽度 4→8px（与 Canvas N6 同向）。markdraw/Excalidraw 往返契约由 T1/T2 契约测试锁定（全绿）。678 测试全绿。T8（bounds/命中）：铅笔颗粒几何常数（散布/半长/半厚）上收 profile 共用真源（sampler 默认值引用之）、铅笔 v2 解析上界 pencilV2VisualHalfWidth（wobble 基底/颗粒外缘/笔端沿切向外伸三者取大 + AA）进 elementVisualBounds 分支，远端湿墨 _strokeBounds 对 v2 style 用同一上界（与 v1 公式取大，保守覆盖缺压感回退）；消费点（scene 命中/选区、export_bounds、viewport_culling、聚焦 dim）经 elementVisualBounds 单一真源自动生效；674 测试全绿（含 T4/T5 的墨迹包络⊆bounds 经验锁与 v1 A20/A21）。T7（远端）：LiveInkStyle renderVersion 全链（发送端 whiteboard_page 显式带 2）、_sameStyle 纳入版本阻断中途翻转、段渲染 v2 分支（双 leading→实测扩到 4 个：桥接边滤波窗口 3 边 + 入界 join 的 from 切线窗口；计划"至少 2 个"允许）、桥接边（连前段末点→本段首点）归较后段（否则两段都不拥有、冻结边界缺 4 个 key）、入界 join 半宽取 from 边（与内部 join 同口径）、冻结整笔末块 ownsStrokeTail（笔尖帽随最后冻结点）、v1 路径输入列表保持单 leading 不变；验收 5 测试：63/64/65/127/128/129 边界段 key 并集=整笔且无重复、远端 vs 静态 90% mask ≤2%（铅笔/毛笔）、block 合并已冻结区域有序一致+边界边集合一致、版本翻转拒段、v1 仍走 adapter。已知口径：分块边界主体 Path 点序与整笔在边界边邻域不同（入界 join 在段首 vs from 边循环尾），点集相同、由 90% mask 像素门净检验；铅笔基底/毛笔包络补"每边终点顶点"保证跨 Picture 边界无白缝（整笔与分块同含该顶点，逐值一致）。T4-C（条件缓存，实测触发）+ T12 机器可执行部分：同机实测 1000 元素静态重绘 v2/v1=4.14 超 1.20 触发线 → 按计划条款落地最小有界缓存 NaturalMediaPathCache（LRU 2048——512 在 1000 元素场景每轮淘汰过半；键含 element id/version/versionNonce/renderVersion/isComplete/strokeWidth/strokeColor/opacity 与 geometryVersion=1，strokeColor/opacity 因烘进缓存 Paint 必须入键；只缓存整笔静态渲染，owned 分段/湿墨调用几何随范围变化不缓存；hit/miss 计数探针 + 编辑后不得回放旧 Picture 测试）。实现迭代：Path 复用 4.14→2.91，改录恒等矩阵 ui.Picture 后 2.29～2.81。缓存首绘缺陷两处（测试暴露后修正）：毛笔 miss 分支只录不绘丢毫丝（N13 远端 35% 透明度像素差 0.050 破 2% 门，正是该对照抓到）与铅笔 miss 分支基底双绘（首绘与命中不一致）——统一改为"录制一次 Picture→存缓存→立即 drawPicture 重放"，miss 与命中重放同一 Picture 对象逐字节一致；同时修正测试自身缺陷（resetForTesting 夹在两次渲染之间会清掉应命中的条目，曾掩盖/误报计数）。门禁修订（计划允许凭实测证据）：1000 元素静态 v2/v1 P95 ≤3.0（实测 2.29～2.81；剩余 ~2.8× 为颗粒栅格化本征成本——每颗粒独立旋转椭圆 path，v1 为单 path 变宽描边，再降须砍粒子密度、牺牲 T0 已批质感且违反"优先调低上限仅在不损目标时"的本意，故以门禁锁上界）。687 测试全绿（685+缓存测试+性能门禁）；analyze 48 基线不变。T12 跨端 Profile（Web/Windows/HarmonyOS）与盲测为用户执行项，待办。T13 收口后复核（用户质询"确定需要修改的文件都修改了吗"）发现并修复 T4-C 一处真缺口：本地湿墨（layered painter 经 ElementRenderer.render 整笔调用，isComplete=false）未绕过缓存——每帧新 FreedrawElement 的 versionNonce 为构造随机值，恰好每帧换键才没触发"第二帧命中首帧 Picture 冻结活动笔迹"，且湿墨期间每帧向 LRU 塞一次性 Picture；缓存条件收紧为 owned==null && isComplete，测试补防冻结断言（同 id 两帧不入库不命中、第二帧像素必须反映新增点）。同轮复核确认接口设计.md 无需改（live ink 字段级协议记录位置在数据模型.md，接口文档只到 {type,payload} 粒度）、conventions.md/架构约束.md/AGENTS.md 不涉及；另修正本行下方"最终提交"的提交清单计数 |
| 最终提交 | 分支自起始 `3eb2b97` 起共 21 个提交（以 `git log 3eb2b97..HEAD` 为准）：`23ac18b` 计划定稿 → 实现链 `732fa68`(T0)…`059bed0`(T4-C/T12) 18 个（含 `4dd3d90`/`bc06ba9` 两枚 lint 与 `f327eee`+`a59c31c` 计划恢复对）→ `7efc473` T13 文档收口 → 末位提交为本地湿墨绕过缓存修复与清单修正；逐任务映射：T0=`732fa68`+`4719264`、T1=`c2aca40`、T2=`9902924`、T3=`69320bf`、T4=`5fbc93a`+`26505be`+`4dd3d90`、T5=`625d142`、T6=`d2a7f46`+`bc06ba9`、T7=`844d0e1`、T8=`84ff8f4`、T9=`f327eee`+`a59c31c`、T10=`abf436b`、T11=`abeeefc`、T4-C/T12=`059bed0`、T13=`7efc473`；分支待用户推送/建 PR |
| renderer 参数 | 铅笔 `PencilStrokeRendererV2.draw(canvas, element, style, {ownedEdgeStart, ownedEdgeEndExclusive, edgeIndexOffset})`；毛笔 `BrushPenStrokeRendererV2.draw(…, {以上 + ownsStrokeHead, ownsStrokeTail})`；整笔静态渲染不传 owned 参数（走 T4-C Picture 缓存），远端湿墨分段按 §3.4 边归属传全局边范围；family 枚举 `strokeRendererFamilyFor(customData)`（core 纯枚举），唯一像素 dispatch 在 `element_renderer.dart`（`pressures.isEmpty → classicV1`） |
| 测试计数 | editor_core + collaboration 套件 687 全绿（T11 时 685，+T4-C 缓存测试与 1000 元素性能门禁各 1） |
| analyze 基线/分支 | 48 issues，与 main 基线一致，无新增 |
| Web build/Profile | 待用户执行：`flutter build web` + 真实 Chrome Profile（自动化侧已做：SVG 经 Chromium 实际栅格验证与 Canvas 同向；种子跨端一致性由 tool/natural_media_hash_web_check dart2js+node(V8) 双侧锁定，见已知降级①） |
| Windows Profile/协作 | 待用户执行：Windows 构建 Profile + 双端协作混合版本对照（v1/v2 客户端同房互绘） |
| HarmonyOS Profile | 待用户执行（不得默认通过）：真机构建 Profile + 手写笔实绘验收（复赛要求，截止 2026-09-30） |
| 盲测结果 | 第一轮（2026-08-30，安卓平板 d6810bf2 真机手写笔，release ea1952e）：铅笔好评（"效果挺好"）；毛笔"不尽人意/真实效果和原型图差多了"。像素级诊断（截图测量+合成复现）定位两个根因并修复（提交 271dc0c）：①折角 join 单宽+双切线体系致变宽折角月牙缝（实测占笔宽 20-45%）→梯形过渡+滤波切线统一+miter 不丢点，合成终检 0 漏底；②真实手写压力活动集中中段（±0.05），原曲线增益 ~0.6 致宽度摆动仅 ±6%（与平板实测吻合）→中心对称凸扩张 γ=0.65（端点量程不变，中段增益 ~3×，实测摆动 27.7-34.4%）。已重新构建部署待用户二轮验证。诊断方法沉淀：inspect_real_input_repro_test（高密度点/窄压力/收笔降压合成复现）——fixture 理想压力曲线与真实输入分布的差异是既有测试的盲区 |
...
| Issue/PR | PR [#22](https://github.com/qinyre/FlowMuse/pull/22) 已合并 main（merge commit `b2e63cb`，2026-08-31 用户确认"提交合并"）；合并前 rebase 处理与智能排版四至七轮（main `6c606f7`）在 whiteboard_page.dart 的一处文案冲突（取 main 侧"没有可智能排版的内容"），合并后 720 测试全绿；远端与本地 feature 分支均已删除 |
