...
日期：2026-09-23
状态：好友与文字私聊阶段已实现并完成本机自动检查；尚未上线，邀请与联合真机验收待完成。分支 `feature/friends-collaboration`，基线 `91cc03a`。按独立可验证任务逐次提交，实际进度见末尾记录。
...
| T04、T10–T11 | 好友/纯文字部分完成；设备和邀请数据模型未开放 |
| T03 | 原语 VM 探针通过，完整 HPKE Auth、独立互测、Web/OHOS 和评审仍未完成 |
| T08–T09、T12–T15 | 等待 T03；没有定向邀请 API、设备信任或邀请卡片实现 |
| T16–T18 | 文字私聊、云端历史、重试、未读、前台恢复与账号隔离完成自动验证 |
| T19–T20 | 好友/文字范围测试通过；Web release、Android debug 构建通过；邀请安全测试和 HAP 构建未通过验收 |
| T21 | 未完成实体设备与真实双账号联合验收 |
| T22 | 已更新功能、架构、接口、数据与隐私说明，提供合成 UI 样例；完整邀请演示待后续 |
| T23 | 未部署新好友模块；总开关默认关闭，已记录发布和回退操作 |

- UI/可靠性：宽窄屏页面、申请/好友/屏蔽管理、设置好友码及白板消息弹层完成。保持原房间与控制器，按气泡可见性更新已读；发送确认不越过补查游标、失败消息保留至账号内存清理。全量 Flutter 1751 通过、5 项既有跳过，专项分析无问题。
