...
日期：2026-09-23
状态：好友、文字私聊、设备核验及定向邀请已实现，真实 HTTP/Socket/数据库邀请联调通过；尚未上线，鸿蒙与实体双端验收交给用户/队员。分支 `feature/friends-collaboration`，基线 `91cc03a`。按独立可验证任务逐次提交，实际进度见末尾记录。
...
| T23 | 未部署新好友模块；总开关默认关闭，已记录发布和回退操作 |

本轮补充：网络结果不明的发送保留原信封和幂等 ID；账号/白板改变即停止应用旧结果；真实白板回归验证弹层不销毁房间、取消加入保留旧画布、joined 回调只在连接就绪后触发。真实联调发现并修复 Socket 加入早于服务端身份查询完成的时序问题，双方以 `init-room` 作为就绪边界。[测试与鸿蒙交接](../research/2026-09-23-invitation-testing.md)包含现成测试账号与独立重现环境；生产没有开启邀请。
...
- CI：Quality 继续使用现有工作流。新增 PostgreSQL service 的配置因当前凭据缺少 `workflow` 权限而未应用，已保留待应用补丁；数据库集成验证来自本次独立测试容器。真实生产数据库没有参与测试，生产配置未开启社交功能。
