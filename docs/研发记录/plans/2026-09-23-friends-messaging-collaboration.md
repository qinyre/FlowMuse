...
日期：2026-09-23
状态：好友、文字私聊、设备核验及定向邀请已实现，真实 HTTP/Socket/数据库邀请联调通过；已按用户要求部署 Web/API，鸿蒙与实体双端验收交给用户/队员。分支 `feature/friends-collaboration`，基线 `91cc03a`。按独立可验证任务逐次提交，实际进度见末尾记录。

2026-09-24 交互调整：用户要求普通好友邀请无需交换安全卡。当前以[简化邀请计划](2026-09-24-simple-friend-invitations.md)为准：设备自动准备，默认使用已鉴权目录，带外核验仅为可选功能。下文 T09、T13、K02、联合脚本与实施记录中的强制核验步骤属于原始方案，已被此调整替代；加密、权限、状态与账号隔离要求继续保留。
...
实现与自动集成验证使用隔离测试数据。用户随后明确要求部署并合并，已执行 Web/API 发布；T21–T23 中尚未完成的移动端验收与回退演练如实保留。
...
用户要求完成定向协作邀请后由其测试，鸿蒙交由队员验证。因此继续 T03、T08/T09、T12–T15 的实现与可用环境验证，鸿蒙构建/真机移交不再阻塞开发。仍保留设备安全卡双向核验、HPKE Auth 和服务端只存密文的边界；未经鸿蒙验证不声称该端验收通过。按密码封装、设备持久化/API、邀请事务/API、客户端设备信任、邀请卡片及白板加入、联合测试分别提交。邀请单独配置开关，允许测试环境启用，不在本次工作中合并或部署生产。

详细证据、截图、复测和上线待办：[阶段验证记录](../research/2026-09-23-social-validation.md)。

| 任务 | 当前状态 |
|---|---|
| T00–T02、T05–T07 | 已实现和自动验证：鉴权、房间归属、好友管理及前端入口 |
| T04、T10–T11 | 设备/邀请幂等迁移、持久卡片、实时提示及独立能力开关完成 |
| T03 | 固定单次 HPKE Auth、RFC seq=0 向量、CIRCL 双向各 16 组互测、浏览器密码探针通过；不实现多消息上下文/exporter；OHOS 与外部安全评审未完成 |
| T08–T09、T12–T15 | 设备安全存储/核验/撤销、邀请事务、卡片及白板加入完成；真实 HTTP/Socket/PostgreSQL/密钥闭环通过 |
| T16–T18 | 文字私聊、云端历史、重试、未读、前台恢复与账号隔离完成自动验证 |
| T19–T20 | 邀请安全与生命周期自动测试通过；Web release、Android debug 构建通过；HAP 由队员构建验证 |
| T21 | 真实后端双测试账号自动联调通过；实体双端/鸿蒙手动验收待用户与队员回传 |
| T22 | 已同步功能、架构、接口、数据与隐私说明，提供测试入口、隔离 Compose 和鸿蒙交接脚本 |
| T23 | 已部署 Web/API 并在生产启用两个开关，数据库/配置/旧镜像已备份，线上冒烟通过；生产回退未演练 |

本轮补充：网络结果不明的发送保留原信封和幂等 ID；账号/白板改变即停止应用旧结果；真实白板回归验证弹层不销毁房间、取消加入保留旧画布、joined 回调只在连接就绪后触发。真实联调发现并修复 Socket 加入早于服务端身份查询完成的时序问题，双方以 `init-room` 作为就绪边界。[测试与鸿蒙交接](../research/2026-09-23-invitation-testing.md)包含隔离测试账号与重现环境；生产邀请已开启，见 [上线记录](../research/2026-09-23-social-deployment.md)。

- UI/可靠性：宽窄屏页面、申请/好友/屏蔽管理、设置好友码及白板消息弹层完成。保持原房间与控制器，按气泡可见性更新已读；发送确认不越过补查游标、失败消息保留至账号内存清理。邀请续作后全量 Flutter 1763 通过、6 项跳过（5 项既有与 1 项 opt-in 联调；联调已单独通过），专项分析无问题。
- 运行时故障：数据库查询失败与会话撤销分开，临时故障返回 503 并跳过未授权通知，不误登出；namespace 被临时拒绝后可由成功 HTTP 刷新重连。Go 及 Dart WebSocket 回归通过。
- CI：Quality 继续使用现有工作流。新增 PostgreSQL service 的配置因当前凭据缺少 `workflow` 权限而未应用，已保留待应用补丁；数据库集成验证来自本次独立测试容器，未用生产数据库运行测试套件。部署时仅执行迁移与线上只读冒烟。
...
- 早期阶段边界（已由本节“邀请实现续作”的用户指令更新）：本机不恢复 DevEco。OHOS 密码探针和 T21 真机门槛仍未满足；已允许在隔离测试环境开放邀请供用户/队员验收，尚未开放生产。
