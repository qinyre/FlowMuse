...
- **R1**（commit 分两个）：① 服务端 confidence 字段 + Dart 模型解析 + 透传 → go/dart
  ...
  测试。
- **R2**：matcher 规则改造 + 专题用例（含 regression："总/结"两簇应被 caption 认领）。
- **R3**：① tokens 文件 + fitFontSize 单测；② 引擎消费接入 + 几何断言；③ 服务端
  规格 v3 字段 + sanitize + 兼容测试。
- **R4**：critique mode 端到端 + 开关 + 超时回退用例（flag 默认关，真机验证后再开）。
