...
- [x] 两人均从同一个最新 `main` 创建功能分支。（2026-08-21 自 `1a56b57` 创建 `feature/ai-multimodal-whiteboard` 与 `feature/harmony-ai-experience`）
...
- [x] 新增 `AiVisualAttachment` 及严格校验。
- [x] 扩展 Agent 请求参数，支持文本和视觉附件同时存在。
- [x] 保持现有文本选区优先、整篇文本回退逻辑。
- [x] 当选区包含笔迹、图片或 PDF 页面时生成视觉上下文。
- [x] 在 AI 面板显示实际上下文范围和附件数量。
- [x] 请求前向用户说明视觉内容将发送到其配置的模型服务。
...
- [x] 搜索并复用现有元素 bounds、PNG/SVG 导出和图片缓存能力。（复用 `MarkdrawController.exportPng(selectedOnly)` / `PngExporter`）
- [x] 支持渲染选中的自由笔迹、图片和 PDF 背景及其上层批注。（`StaticCanvasPainter` 统一渲染）
- [x] 保持元素比例、文字清晰度和公式可识别性。（scale=2 渲染后等比缩放）
- [x] 对超大区域进行缩放或分块，禁止直接发送未压缩大图。（最长边 ≤2048 等比缩放重编码）
- [x] 渲染过程不得修改 Scene、History 或当前选区。
- [x] 取消请求后释放临时字节引用。（附件仅存活于单次请求）
...
- [x] 扩展 `AiAgentRepository.run()` 的消息构造。
- [x] 文本上下文继续作为不可信 JSON 数据传递。
- [x] 图片使用 OpenAI 兼容的多模态 content 结构。（`image_url` + data URL）
- [x] 保留 `NativeHttpClient`、鸿蒙原生 HTTP、取消令牌和超时机制。
- [x] 模型不支持视觉输入时给出明确错误，而不是显示通用失败。（HTTP 400 分类提示）
- [x] 为 Repository 增加最小可测试的 HTTP 注入点，避免引入完整网络框架。（`AiAgentHttpPost` typedef）
...
- [x] 保持 `rename_note`、`insert_text`、`generate_mindmap`、`smart_layout` 四种白名单 action，不新增学习类 action。（parser/校验器零改动，回归测试通过）
- [x] 多模态上下文下继续复用同一 action parser 和严格校验器。
- [x] 用户编辑预览后重新走同一校验器。
- [x] 拒绝未知字段、非法 JSON、超长内容和超量导图节点。

### A5. 确定性白板落地

- [x] 复用 `insertPlainTexts`、`MindmapLayout`、分页追加和 `CompoundResult`。（v1 已实现，2026-08-21 勘察确认）
- [x] 将 AI 整理出的文本或导图作为普通元素一次性提交。（`_applyAiAgentResponse` 复合提交，含失败回滚重命名）
- [x] 分页空间不足时只追加实际需要的页面。（speech_text_insertion_test 覆盖）
- [x] AI 结果继续落为普通白板元素，参与保存、导出和协作同步。（既有链路零改动）
...
- [x] 选区存在时提供"解释这里""检查公式""整理文字""整理成导图"。
- [x] 显示准备选区、理解内容、生成结果、排版等阶段状态。（客户端可知阶段近似；真实分阶段需 Repository 进度回调，后续按需引入）
- [x] 鸿蒙平板使用响应式侧栏，不固定在过小的最大尺寸。（宽屏 ≥900px 时面板宽 420）
- [x] 保留预览、编辑、逐项选择、追问、取消和应用后继续对话。
- [x] AI 写入继续要求用户确认，不自动修改笔记。

### A7. A 线测试

- [x] 多模态附件数量、MIME、尺寸和总大小限制测试。
- [x] 多模态 Chat Completions 请求契约测试。
- [x] 多模态上下文下受限 action 的合法与非法结构测试。（parser 零改动，既有结构测试回归通过）
- [x] 思维导图深度、节点数和文本长度限制测试。（既有 ai_agent_models_test）
- [x] AI 写入一次落地、一次撤销测试。（既有 mindmap/speech 插入测试）
- [x] 分页空间不足时追加页面测试。（既有 speech_text_insertion_test）
- [x] 面板选区范围、预览编辑和取消测试。（新增选区快捷指令与阶段状态用例）
- [x] 现有 AI 助手、思维导图和语音文本插入测试保持通过。（全量 flutter test 361 通过）
...
- [x] A1 多模态上下文模型。（2026-08-21 完成，`feature/ai-multimodal-whiteboard`）
- [x] A2 选区视觉附件生成。
- [x] A3 多模态请求。
- [x] A4 受限整理动作接线。
...
- [x] A5 确定性白板落地。（2026-08-21 验证：v1 实现加既有测试覆盖，零新代码）
- [x] A6 AI 面板体验。
- [x] A7 Flutter 测试。
