...
本次默认 release 构建在 `ProcessRouterMap` 阶段因本机 ohpm 依赖解析 `ENOENT: stat ''` 中止；针对真机调试的 `flutter build hap --debug --no-pub` 已成功。设备原有 FlowMuse 为旧 `app-identifier` 和旧证书，`hdc install -r` 返回 `9568332 install sign info inconsistent`，未覆盖安装。用户确认该测试设备无须保留旧应用数据后，已卸载旧包并安装新 HAP；设备 `bm dump` 显示 APP ID、`client_id` 都是 `6917611606499874343`，开发证书指纹为上述 `5215...`。

后续用户真机点击“华为账号登录”反馈登录成功，设置页显示“已通过华为账号登录 · 尚未绑定邮箱”。按当前代码，该状态来自服务端验证华为授权码并签发的业务会话；这一反馈证明登录主流程已可用。登录时未弹出账号页面与设备已有账号或已有授权后的直接返回行为相符。当前原生请求只获取授权码，未申请 `profile` 范围；服务端新建华为用户时默认昵称是“华为用户”，头像为空，所以显示默认资料是预期现状。用户尚未反馈“华为账号”绑定栏的显示或取消授权等分支的真机结果。
