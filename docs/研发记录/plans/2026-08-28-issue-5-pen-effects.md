...
3. 毛笔：
   ...
   - 分段渲染语义（TaperPhase，见 §6.1）：本地湿墨与静态元素 = full；远端冻结首块/首段 = headOnly；远端最新尾段 = tailOnly（taper 跟随对端笔尖，段冻结后去掉 end taper 是正确行为）；其余段 = none。若不做此区分，毛笔会在每个 64 点段边界周期性收针；
...
- 毛笔收锋（绝对距离判据，替换原弧长百分比判据——后者与绝对 taper 数学不相容）：对折线长 ≥ 20×size 的恒压 fixture，距起点 1×size 与距终点 1×size（沿原始折线弧长）处局部宽度 ≤ 中段（50% 弧长处）宽度的 65%，距起/终点 2×size 处 ≤ 85%；
...
1. 本地湿墨 -> 静态元素：
   - 五种笔刷的 profile 和轮廓一致；
   - terminal 湿墨与静态元素 bounds 每边误差不超过 1 个逻辑像素（严格口径用于圆珠笔/钢笔等无 taper 笔型；毛笔等 taper 笔型用 ≤ size/8 分层口径，isComplete 末端删点为已知差异来源）；
...
| A14 | 湿墨提交 | strokeWidth=4 时 terminal 湿墨与静态 bounds 每边误差 ≤1 逻辑像素；大宽度 ≤ size/8（isComplete 末端删点为已知差异来源） |
