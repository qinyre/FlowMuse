...
- **滤波坐标系固定为 EditorCanvas local logical pixels**：先在 localPosition 空间完成建模，再由 controller 用**不取整的** screen→scene 变换保存为浮点 scene coordinates。自由书写不得调用会 `roundToDouble()` 的通用 `screenToScene`，否则会重新引入 1 scene-pixel 量化噪声；其他需要整数对齐的工具可继续使用旧变换。这样 One Euro 和最小距离参数不随 viewport zoom 改变；录制文件同时保存 viewport 元数据用于回放。
...
- `quadratic`：perfect-freehand 官方二次中点法——从 `outline[0]` 起笔，**每个** outline 点（含 `outline[0]`）各作为一次控制点、以它和下一点（最后一点的下一点为第一点）的中点为终点，构造连续二次贝塞尔闭合路径；
...
- 二次闭合路径必须显式处理最后一点到第一点的接缝，不能遗漏 `outline[0]` 控制段；除 seam、自交和小点数测试外，还必须用 goldens/真机 A/B 覆盖接缝、端帽和曲线轮廓。
...
另加亚像素回归：在非整数缩放和含小数 localPosition 的样本中，提交后的 freedraw points 必须保留小数 scene 坐标，不能因坐标变换退化为整数网格。
