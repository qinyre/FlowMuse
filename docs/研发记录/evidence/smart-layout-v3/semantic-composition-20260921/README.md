...
首轮范围：计划 A+B，代码 `5601f36` 的历史实机记录见下。PR #39 未合并。本轮 C 协议 `2bf9601`、消费 `126b5a2` 已推送，D 离线检查通过。后端已升级 C 协议，在线验收发现的问题仍在修复，平板尚待升级；以下旧实机数据不是 C/D 验收。

## C/D 第二轮离线检查点

- 评分改为实际 reduced Scene + render snapshot；输入侧冻结关系分母，缺测/不适用分开，原稿只读比较。完整可比较且改善至少 0.05 才默认推荐，否则默认保留原样；备选可手选，没有伪造的 no-op patch。
- 全量 Flutter：1753 通过、2 条条件跳过（已有环境测试、显式开启的在线验收）。12 页效果集反复生成确定，集中测试共 23 项通过。缺资产页不编造改善差值，已排好页默认保留；伪造 placement 数据不能改变实际指标。
- 同机 Excalifont、一次预热十次本地候选（不含网络/构造样例）：新耗时微秒 `[39154,38256,37479,35114,36010,35929,34592,36251,34556,34154]`，中位数 35.970ms、最大 39.154ms；相对 D 前 30.806/35.890ms，增加约 5.164/3.264ms（包含原稿额外只读渲染）。未据十次样本宣称 p95。
...
| 02 交叉摆放 | mediaSide；按预给关系复原两组，图文左右对应，没有把空间邻近当真值 |
