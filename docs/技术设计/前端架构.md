...
元素在创建时盖上不可变归属戳,存在 Excalidraw `customData.flowMuse.collaborationOwner`(v1:creatorKey/displayName/isGuest):登录用户的 creatorKey 由 userId 哈希派生(跨房间稳定),游客用房间内会话 key;更新元素保留原作者,复合结果(思维导图、绑定文本)整体盖当前操作者。三类易失 presence(MOUSE_LOCATION/IDLE_STATUS/USER_VISIBLE_SCENE_BOUNDS)的密文正文带可选 `creatorKey`,让协作者头像和远端湿墨在重连后仍能对上同一个人(ADR-018,服务端零改动,旧客户端忽略新字段)。

点击顶部协作者头像可按创建者聚焦:其他人的元素按原 z 序以 0.22 alpha 变淡,PDF 底稿、分页纸张、网格、协作光标、选择框、本地活动笔迹与正在编辑的内容保持全亮。聚焦是纯本机视图状态——不进 Scene、历史、SQLite 和网络,也不限制编辑;外部导出(`.excalidraw`/`.json`)自动剥离归属字段,协作密文与本地存储保留。
