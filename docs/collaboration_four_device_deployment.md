...
Android 平板 / 鸿蒙平板
        |
        | http://<Linux 公网 IP>:3000
        v
Linux 服务器公网入口
        |
        | frp TCP 转发
        v
Windows 开发机 127.0.0.1:3000
        |
        | Go 协作后端
        v
Windows 本机 PostgreSQL + MinIO
```
...
| Linux 服务器 | `3000` | 对外暴露 FlowMuse 协作服务 |
| Windows 开发机 | `3000` | Go 协作后端 |
| Windows 开发机 | `5432` | PostgreSQL |
| Windows 开发机 | `9000` | MinIO S3 API |
| Windows 开发机 | `9001` | MinIO 控制台 |

Linux 防火墙和云厂商安全组至少放行 `7000` 和 `3000`。
...
```text
postgres://flowmuse:flowmuse_dev_password@127.0.0.1:5432/flowmuse?sslmode=disable
```
...
minio.exe server D:\FlowMuseData\minio --console-address ":9001"
```
...
```powershell
$env:FLOWMUSE_ADDR=":3000"
$env:DATABASE_URL="postgres://flowmuse:flowmuse_dev_password@127.0.0.1:5432/flowmuse?sslmode=disable"
$env:FLOWMUSE_S3_ENDPOINT="127.0.0.1:9000"
...
如果服务器使用云厂商安全组，也要在安全组里放行 `7000/tcp` 和 `3000/tcp`。
...
```powershell
Invoke-WebRequest -Uri "http://<Linux 公网 IP>:3000/health" -UseBasicParsing
```

Android 平板和鸿蒙平板也应能在浏览器打开：

```text
http://<Linux 公网 IP>:3000/health
```
...
客户端协作服务地址由 `FLOWMUSE_COLLAB_SERVER_URL` 指定。移动设备不能使用默认的 `127.0.0.1`，必须使用 Linux 公网 IP。
...
```powershell
& "C:\tools\flutter_ohos\bin\cache\dart-sdk\bin\dart.exe" "C:\tools\flutter_ohos\bin\cache\flutter_tools.snapshot" run --dart-define=FLOWMUSE_COLLAB_SERVER_URL=http://<Linux 公网 IP>:3000
```

构建 debug APK：

```powershell
& "C:\tools\flutter_ohos\bin\cache\dart-sdk\bin\dart.exe" "C:\tools\flutter_ohos\bin\cache\flutter_tools.snapshot" build apk --debug --dart-define=FLOWMUSE_COLLAB_SERVER_URL=http://<Linux 公网 IP>:3000
```
...
如果使用 DevEco Studio 安装鸿蒙包，也要确保构建配置里包含：

```text
--dart-define=FLOWMUSE_COLLAB_SERVER_URL=http://<Linux 公网 IP>:3000
```

两个平板必须使用同一个 `FLOWMUSE_COLLAB_SERVER_URL`。
...
长期方案建议在 Linux 上用 Nginx 或 Caddy 提供 HTTPS，再反向代理到 `127.0.0.1:3000` 或 frp 暴露端口。当前阶段为了快速验证协作能力，可以先使用 HTTP 公网 IP。
...
4. Linux 启动 `frps`。
5. Windows 启动 `frpc`。
6. 两台平板浏览器分别打开 `http://<Linux 公网 IP>:3000/health`，确认可访问。
7. Android 平板安装或运行带公网 `--dart-define` 的 FlowMuse 客户端。
8. 鸿蒙平板安装或运行带同一个公网 `--dart-define` 的 FlowMuse 客户端。
...
- Linux 防火墙是否放行 `3000/tcp`
- 云服务器安全组是否放行 `3000/tcp`
- `frps` 是否正在运行
- `frpc` 是否连接成功
- `frpc.toml` 的 `localPort` 是否是 `3000`

### 8.2 公网 `/health` 可访问，客户端无法创建房间

优先检查：

- 客户端是否使用了 `--dart-define=FLOWMUSE_COLLAB_SERVER_URL=http://<Linux 公网 IP>:3000`
- 移动系统是否拦截明文 HTTP
