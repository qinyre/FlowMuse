...
客户端协作服务地址由 `FlowMuse-App/.env` 中的 `FLOWMUSE_COLLAB_SERVER_URL` 指定。移动设备不能使用默认的 `127.0.0.1`，必须在构建前把 `.env` 改成 Linux 公网 IP。
...
```dotenv
FLOWMUSE_COLLAB_SERVER_URL=http://<Linux 公网 IP>:3000
```
...
- `FlowMuse-App/.env` 是否已经改成 `FLOWMUSE_COLLAB_SERVER_URL=http://<Linux 公网 IP>:3000`
