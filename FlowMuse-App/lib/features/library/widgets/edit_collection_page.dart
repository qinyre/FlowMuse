...
class EditCollectionParams {
  const EditCollectionParams({
    required this.id,
    required this.name,
...
class EditCollectionPage extends StatefulWidget {
  const EditCollectionPage({super.key, required this.params});

  final EditCollectionParams params;
...
class _EditCollectionPageState extends State<EditCollectionPage> {
  ...
  void initState() {
    super.initState();
    _controller = TextEditingController(text: widget.params.name)
      ..addListener(_onTitleChanged);
    _focusNode = FocusNode();
    _selectedColor = widget.params.coverColor;
    _selectedCoverImage = widget.params.coverImage;
    _loadCovers();
    _loadRecentCovers();
  }
  ...
  void _save() {
    ...
    if (_selectedCoverImage != null) {
      defaultRecentCoversRepository.addRecentCover(
        widget.params.coverCategory,
        'image',
        _selectedCoverImage!,
      );
    } else {
  ...
  Future<void> _loadRecentCovers() async {
    final recent = await defaultRecentCoversRepository
        .getRecentCovers(widget.params.coverCategory);
    if (mounted) {
      setState(() {
  ...
  Future<void> _clearRecentCovers() async {
    await defaultRecentCoversRepository
        .clearRecentCovers(widget.params.coverCategory);
    if (mounted) {
      setState(() {
        _recentCovers = [];
      });
...
class _RecentCoverChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final color =
        isColor ? Color(int.parse('0xFF${item.value}', radix: 16)) : null;
