...
- 信任边界:服务端不保存明文密钥。2026-09-24 起好友邀请默认信任已鉴权设备目录的公钥归属，独立设备身份核验为可选操作；默认模式不承诺防御恶意目录替换公钥，详见 `docs/研发记录/plans/2026-09-24-simple-friend-invitations.md`。
...
- 服务端 JIIX 多元素相对坐标缺陷仍需另行评审（见设计稿 §2）。智能排版同款 max 缺陷已在 Issue #14 修复：旧版转写创建、模板改字号/加项目符号、草稿校对共用紧框规则，仍保留原字号范围、手动定宽/绑定文本与公式/竖排例外；V3 不变（见 `docs/研发记录/plans/2026-09-20-issue-14-text-box-sizing.md`）。
