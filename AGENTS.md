...
| `docs/参赛文档/`            | 复赛演示视频工程入库，其余竞赛过程材料本地忽略                    |
...
- 鸿蒙网络安全：生产协作服务使用 HTTPS；`network_config.json` 的 cleartext HTTP 放行保留用于局域网/自建服务调试。
...
- HTTP 通道：`shared/network/native_http_client.dart` + `HttpChannel.ets`（手写识别目录保留兼容 export）
- 华为账号：`huawei_account_channel_ohos.dart` + `HuaweiAccountChannel.ets`（`flow_muse/huawei_account`，服务端校验授权码后才创建业务会话）
